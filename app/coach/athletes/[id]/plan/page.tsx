...
import { generatePlan, weeksUntil, type WorkoutType } from '@/lib/plan-generator';
...
export default async function AthletePlanPage({ params }: { params: { id: string } }) {
  ...
  if (!athlete.vdot) {
    return (
      <>
        <Link href={`/coach/athletes/${athlete.id}`} className="inline-flex items-center gap-2 text-slate-400 hover:text-white mb-6">
          <ArrowLeft className="w-4 h-4" />
          Voltar
        </Link>
        <div className="glass rounded-2xl p-12 text-center">
          <p className="text-slate-400">
            Este atleta ainda não tem VDOT calculado. Informe uma prova recente para gerar o plano.
          </p>
        </div>
      </>
    );
  ...
  // Agrupa semanas por fase para exibição.
          ...
          <section key={`${group.phase}-${group.weeks[0].weekNumber}`}>
            ...
            <div className="space-y-4">
              {group.weeks.map((week) => (
                <div key={week.weekNumber} className="glass rounded-2xl p-5">
                  ...
                  <div className="space-y-2">
                    {week.workouts.map((w) => (
                      ...
                      >
                        ...
                        <div className={`shrink-0 px-2 py-0.5 rounded text-xs font-bold ${typeColors[w.type]}`}>
                          {w.type}
                        </div>
